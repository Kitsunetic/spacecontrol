...
class TrellisTextTo3DPipeline(Pipeline):
    ...
    def encode_spatial_control(self, spatial_control_path: str) -> torch.Tensor:
        ...
        utils.save_voxelgrid_as_ply(
            spatial_control[0, 0].cpu().numpy(), Path(spatial_control_path).parent / "spatial_control_voxlized.ply"
        )
        utils.save_voxelgrid_as_ply(
            spatial_control_latent[0].cpu().numpy(), Path(spatial_control_path).parent / "spatial_control_latent.ply"
        )
